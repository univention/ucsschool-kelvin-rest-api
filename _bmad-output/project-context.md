...
- Persistence modeling uses SQLAlchemy 2 Declarative models in ucsschool-objects, including first-class Role and SchoolMembership entities and dedicated association tables.
- Synchronization mapping tables exist for DN-to-public_id linking (school, user, group) via kelvin-connector models and Alembic-managed schema.
...
- Preserve current domain dataclass contract shape in ucsschool-objects:
	- School, Group, User, Role, and SchoolMembership are domain-level classes.
	- UNLOADED and UNSET sentinels represent intentionally unloaded and intentionally unset values.
...
- Preserve membership semantics on domain User models:
	- school_memberships is keyed by school public_id (dict[UUID, SchoolMembership]).
	- derived properties (primary_school, groups, roles) are computed from membership values, not parallel cached fields.
	- preserve adapter field-name translations where ORM and domain names differ (for example Group.has_share -> Group.create_share).
	- Group.members is a domain-level set[User] projection backed by school_membership and group_member_association persistence joins.
...
- For ucsschool-objects model and adapter tests, keep ORM-internal assertions and domain-level assertions separated (no persistence leakage across port contracts).
...
- For membership tests, preserve invariants around primary school handling:
	- persistence enforces at most one primary school membership per user through primary_user_constraint uniqueness.
	- domain User.primary_school expects a primary membership and raises ValueError when none exists.
	- test happy path, duplicate-primary invariant violations, and no-primary error-path behavior.
...
- Current Alembic chain for the relational model:
	- f1c5bf519a40 initializes core tables and associations.
	- e8b27dd51414 adds DN/public_id mapping tables.
	- a3f9c12e8b01 seeds default role rows used by role-aware flows.
...
- Do not regress ORM invariants introduced in the current schema:
	- preserve unique constraints on (record_uid, source_uid) for school, group, and user.
	- preserve unique user-school membership and the single-primary-membership invariant.
- Respect relationship loading intent in SQLAlchemy adapter code:
	- many relationships intentionally use lazy="raise" to prevent accidental implicit loading.
	- selectin loading is used intentionally on selected relations (for example membership->user/school and group->school).
