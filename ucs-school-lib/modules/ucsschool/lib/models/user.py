...
class User(RoleSupportMixin, UCSSchoolHelperAbstractClass):
    ...
    async def remove_from_groups_of_school(self, school: str, lo: UDM) -> None:
        ...
        for cls in (SchoolClass, WorkGroup, SchoolGroup):
            for group in await cls.get_all(lo, school, filter_format("uniqueMember=%s", (self.dn,))):
                try:
                    udm_obj.props.groups.remove(group.dn)
                    self.logger.info(
                        "Removing group %r of school %r from the user %r.", group.dn, school, self.dn
                    )
                except ValueError:
                    self.logger.info(
                        "Group %r of school %r already no longer assigned to user %r.", group.dn, school, self.dn
                    )
        ...
        if await self.is_administrator(lo):
            ...
            for dn in admin_group_dns:
                try:
                    udm_obj.props.groups.remove(dn)
                    self.logger.info(
    ...
    def get_legal_guardians_groups(self, schools=None):  # type: (Optional[List[str]]) -> List[str]
        prefix = ucr.get("ucsschool/ldap/default/groupprefix/legal_guardians", "sorgeberechtigte-")
        return [
            self.get_group_dn("%s%s" % (prefix, school), school) for school in (schools or self.schools)
        ]
