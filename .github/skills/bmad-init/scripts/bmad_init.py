...
def cmd_load(args):
    ...
    if not project_root:
        print(
            json.dumps({"error": "Project root not found (_bmad folder not detected)"}), file=sys.stderr
        )
        sys.exit(1)
...
def cmd_resolve_defaults(args):
    ...
    if not module_def:
        print(
            json.dumps({"error": f"Failed to parse module.yaml at: {target_yaml_path}"}), file=sys.stderr
        )
        sys.exit(1)
...
def cmd_write(args):
    ...
    for module_code, module_answers_raw in answers.items():
        ...
        module_name = (
            module_def["meta"].get("name", module_code.upper()) if module_def else module_code.upper()
        )
