...
def test_first_reconnect_attempt_is_immediate_then_backs_off():
    ...
    sleeps = []
    with (
        patch.object(SyncStrategy, "_open_socket", side_effect=LDAPSocketOpenError("server down")),
...
def test_get_uldap_conf_props(temp_file_func, random_name):
    ...
    with (
        patch.object(ucsschool.kelvin.ldap, "CN_ADMIN_PASSWORD_FILE", tmp_file1),
        patch("ucsschool.kelvin.ldap.MACHINE_PASSWORD_FILE", tmp_file2),
...
def test_udm_kwargs_fake(temp_file_func, random_name):
    ...
    tmp_file2 = temp_file_func()
    with (
        patch("ucsschool.kelvin.ldap.CN_ADMIN_PASSWORD_FILE", tmp_file1),
        patch("ucsschool.kelvin.ldap.MACHINE_PASSWORD_FILE", tmp_file2),
        patch("ucsschool.kelvin.ldap._udm_kwargs", {}),
    ):
        ucsschool.kelvin.ldap.get_uldap_conf.cache_clear()
