...
async def test_search_multiple_names(
    ...
    """``name`` may be repeated to retrieve several classes in one request."""
    if api_version != "v2":
        pytest.skip("Repeating the 'name' parameter is a v2 feature.")
    ou = await create_ou_using_python()
    _dn1, attr1 = await new_school_class_using_lib(ou)
    _dn2, attr2 = await new_school_class_using_lib(ou)
    _dn3, attr3 = await new_school_class_using_lib(ou)
...
async def test_get_is_case_insensitive(
    ...
    """A school class name is matched regardless of its capitalization, as the search
    is, but exactly: ``*`` in it is not a wildcard."""
    ou = await create_ou_using_python()
    _dn, attrs = await new_school_class_using_lib(ou)
    name = attrs["name"]
    ...
    async def _check():
        response = retry_http_502(
            requests.get,
            f"{url_fragment}/classes/{ou}/{name.upper()}",
            headers=auth_header,
        )
        assert response.status_code == 200, (response.reason, response.content)
        ...
        response = retry_http_502(
            requests.get,
            f"{url_fragment}/classes/{ou}/{name[:-1]}*",
            headers=auth_header,
        )
...
async def test_search_udm_error_forwarding(
    auth_header,
    retry_http_502,
    url_fragment,
    udm_kwargs,
...
async def test_search_leaves_the_members_out_when_excluded(
    ...
    await new_school_class_using_lib(ou, users=[user.dn for user in users])
