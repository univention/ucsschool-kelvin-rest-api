...
async def _apply_user_patch(
    model: UserModel,
    patched: UserPatchDict,
    current: UserPatchDict,
    session: AsyncSession,
    ...
    if modifies_memberships:
        await _apply_membership_relation_changes(
            model,
            current.get("school_memberships", {}),
            patched.get("school_memberships", {}),
            session,
        )
    if modifies_guardians:
        await sync_collection(
            session,
            patched.get("legal_guardians", []),
            current.get("legal_guardians", []),
            UserModel,
    ...
    if modifies_wards:
        await sync_collection(
            session,
            patched.get("legal_wards", []),
            current.get("legal_wards", []),
            UserModel,
...
def _classify_patch_operation(parts: list[str]) -> str | None:
    ...
    reach into a related object's attributes are rejected. The one
    membership attribute that is patchable is ``is_primary`` — it lives on
    the membership itself, not on a related object.
    ...
    if top == "school_memberships":
        if depth <= 2:
            ...
            return top
        if depth == 3 and parts[2] == "is_primary":
            # /school_memberships/<school_id>/is_primary — move the primary flag
            return top
        if depth <= 4 and parts[2] in ("groups", "roles"):
            ...
            # (likewise for .../roles)
            return top
        # Deeper paths target another membership attribute (e.g. school)
