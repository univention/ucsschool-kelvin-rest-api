...
"""add reverse association, school foreign key and lower(name) indexes
...
``ix_user_name_lower`` and ``ix_group_name_lower`` are expression indexes on
``lower(name)``. The case-insensitive name lookups of the v2 API compare
``lower(name)``, which neither the unique index on ``name`` nor a trigram
index can serve. They are declared in ``database_models.py`` too, but
autogenerate does not emit expression indexes, so they are listed here by
hand.
...
_INDEXES: list[tuple[str, str, list[str | sa.TextClause]]] = [
    (
    ...
    ("ix_user_name_lower", "user", [sa.text("lower(name)")]),
    ("ix_group_name_lower", "group", [sa.text("lower(name)")]),
]
...
def _drop_invalid_leftovers() -> None:
    ...
    invalid: Sequence[str] = bind.execute(_INVALID_INDEX_QUERY, {"names": names}).scalars().all()
