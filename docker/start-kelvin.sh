...
while [[ ! -f "/etc/machine.secret" ]]; do
    if [[ $elapsed -ge $TIMEOUT ]]; then
        echo "ERROR: timed out waiting for /etc/machine.secret after ${TIMEOUT}s" >&2
        exit 1
    fi
    echo "waiting for /etc/machine.secret (${elapsed}s / ${TIMEOUT}s)" >&2
    sleep 1
    ((elapsed++))
done
...
if [[ -z "${UCSSCHOOL_KELVIN_DB_URI}" ]]; then
    ...
    while [[ ! -f "$DB_URI_FILE" ]]; do
        if [[ $elapsed -ge $TIMEOUT ]]; then
            echo "ERROR: timed out waiting for $DB_URI_FILE after ${TIMEOUT}s" >&2
            exit 1
        fi
        echo "waiting for $DB_URI_FILE (${elapsed}s / ${TIMEOUT}s)" >&2
        sleep 1
        ((elapsed++))
    done
fi
...
if [[ $? -eq 1 ]]; then
    echo "Module openapi_client_udm is not installed. Installing..." >&2
