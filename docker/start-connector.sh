...
if [ "$LDAP_SERVER_TYPE" != "master" ]; then
    ...
    while [[ ! -f "$APP_PROVISIONING_CONFIG_FILE" ]]; do
        if [[ $elapsed -ge $TIMEOUT ]]; then
            echo "ERROR: timed out waiting for $APP_PROVISIONING_CONFIG_FILE after ${TIMEOUT}s" >&2
            exit 1
        fi
        echo "waiting for $APP_PROVISIONING_CONFIG_FILE (${elapsed}s / ${TIMEOUT}s)" >&2
        sleep 1
        ((elapsed++))
    done
    ...
    while ! python3 -c "import urllib.request, sys; urllib.request.urlopen(sys.argv[1])" "$KELVIN_HEALTH_URL" > /dev/null 2>&1; do
        if [[ $elapsed -ge $TIMEOUT ]]; then
            echo "ERROR: timed out waiting for $KELVIN_HEALTH_URL after ${TIMEOUT}s" >&2
            exit 1
        fi
        echo "waiting for $KELVIN_HEALTH_URL (${elapsed}s / ${TIMEOUT}s)" >&2
        sleep 1
        ((elapsed++))
    done
