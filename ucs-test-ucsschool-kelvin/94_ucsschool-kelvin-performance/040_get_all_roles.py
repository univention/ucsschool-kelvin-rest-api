...
## -*- coding: utf-8 -*-
## desc: Test performance of GET /ucsschool/kelvin/<version>/roles/
...
test_parameter = PerformanceTestParameters(
    target_locust_class="GetAllRoles",
    target_locust_file_name="roles.py",
    route="roles/",
    result_files_name="040-get-all-roles",
...
def test_rps(api_version, check_rps):
    check_rps(
        test_parameter.result_file_base_path(api_version), test_parameter.url_name(api_version), 16
    )


def test_99_percentile(api_version, check_99_percentile):
    check_99_percentile(
        test_parameter.result_file_base_path(api_version), test_parameter.url_name(api_version), 250
    )
