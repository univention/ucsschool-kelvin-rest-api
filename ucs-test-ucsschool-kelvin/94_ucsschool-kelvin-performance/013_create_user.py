...
## -*- coding: utf-8 -*-
## desc: Test performance of POST /ucsschool/kelvin/<version>/users/
...
def test_rps(api_version, check_rps):
    check_rps(
        test_parameter.result_file_base_path(api_version), test_parameter.url_name(api_version), 1.0
    )


def test_99_percentile(api_version, check_99_percentile):
    check_99_percentile(
        test_parameter.result_file_base_path(api_version), test_parameter.url_name(api_version), 5000
    )
