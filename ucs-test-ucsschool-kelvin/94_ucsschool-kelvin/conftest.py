    ...
    def _func(import_user, resource, source="LDAP"):
        ...
        for k, v in resource.items():
            if k == "url":
                ...
                import_user_classes = getattr(import_user, k)
                if source == "LDAP":
                    val = {
                    ...
                    import_user_classes = {
                        school: set(classes) for school, classes in import_user_classes.items()
                    }
                else:
                    val = v
                msg = (
                    "Value of attribute {!r} in {} is {!r} and in resource is "
                    "v={!r} -> val={!r} ({!r}).".format(k, source, import_user_classes, v, val, dn)
                )
                assert import_user_classes == val, msg
