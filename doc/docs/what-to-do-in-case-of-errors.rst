...
.. code-block:: json
   :caption: Example of an HTTP reponse when UDM, a service Kelvin depends on, is unavailable (HTTP status code: :code:`503`).
...
.. code-block:: json
   :caption: Example of an HTTP response to a :code:`POST` request against :code:`/ucsschool/kelvin/v1/users/` when the request body does not conform with the route model and is not processable (HTTP status code :code:`422`). It tells the user that :code:`firstname` is a required field and is missing from the request body.
...
.. code-block:: json
   :caption: Example of an HTTP response to a misconstructed :code:`PATCH` request which adds a student to a school class in ``DEMOSCHOOL`` but also removes the student from ``DEMOSCHOOL``. (HTTP status code :code:`422`)
...
The log file ``/var/log/univention/ucsschool-kelvin-rest-api/http.log`` contains log message of the Kelvin API.
...
Relevant logs for the UDM REST API can be found in ``/var/log/univention/directory-manager-rest.log``.
...
The typcal log statement parts for Kelvin are:
...
The typcal log statement parts for UDM are:
...
To give an example how an Administrator could gather all available information,
...
Looking at the log files, with

.. code-block:: bash

   grep .*89bbd1f5fe /var/log/univention/ucsschool-kelvin-rest-api/http.log

We can see that it happens during an update of the :code:`demo_student` user within the UDM service.
...
We can see now that there is an uncaught exception:
...
Opening the /var/log/univention/directory-manager-rest.log will reveal the full Python traceback,
which, at the end, contains the error message and source file path.
...
.. TODO: What should the admin do now? -> More Info, Support page

.. TODO: reference to the UDM REST API :external+uv-ucsschool-manual:doc:`UDM REST API<udm-rest-api>`
